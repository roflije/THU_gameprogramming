...
class TD_AlienPieces extends A_GameObject {
	private static final B_Shape SHAPE = new B_Shape(8, Color.red);

	private double lifeTime = 1000;
	private TD_AlienAI target;
	private A_Type owner;

	public TD_AlienPieces(A_Type owner, double x, double y, double xDest, double yDest) {
		super(x, y, Math.atan2(yDest - y, xDest - x), 500, SHAPE);
		this.owner = owner;
		this.isMoving = true;
	}

	public TD_AlienPieces(A_Type owner, double x, double y, double a, double s, double time) {
		super(x, y, a, s, SHAPE);
		lifeTime = time;
		this.isMoving = true;
		this.owner = owner;
	...
	public TD_AlienPieces(A_Type owner, double x, double y, TD_AlienAI target) {
